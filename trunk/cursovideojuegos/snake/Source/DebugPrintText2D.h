...
public:
	...
	void	SetFont		( LPDIRECT3DDEVICE9 D3Ddevice );
	// Funcion que hemos de llamar cada vez que queramos pintar un texto.
	int		PrintText	( int x, int y, DWORD color, const char * format, ... );

private:

	LPD3DXFONT	m_font;	//variable con la informacion del tipo de letra a usar.
};
